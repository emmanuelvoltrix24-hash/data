...
Polls latestresult.jsp and outputs each new round as JSON with:
- round_id, week, season, timestamp
- matches: home/away team, odds (H/D/A), result (when available)
- standings: position, points, home/away stats, form (derived)
"""
import requests
...
def form_from_stats(hw, hd, hl, aw, ad, al, played):
    """Derive win rate as a simple form score."""
    total_w = hw + aw
    total_d = hd + ad
    return round((total_w * 3 + total_d) / max(played * 3, 1) * 100, 1)  # % of max points
    ...
    if league is not None:
        for t in league.findall('team'):
            ...
            standings[tid] = {
                ...
                'gd': int(t.get('gd', 0)),
                'form_pct': form_from_stats(hw, hd, hl, aw, ad, al, played),
            }
...
def save_standings(data):
    ...
    sf = f"{SAVE_DIR}/standings_w{week}.json"
    with open(sf, 'w') as f:
        json.dump({'week': week, 'season': data['results']['season'], 'standings': data['standings']}, f, indent=2)
    return sf
...
def display(data):
    ...
    print(f"\n[{ts}] NEW ROUND — week={res['week']} round={res['round_id']}")
    print(f"\n  RESULTS:")
    for m in res['matches']:
        print(f"  M{m['n']}: {m['home']:<20} vs {m['away']:<20} | {m['result']} ({m['parity'] or '-'}) {m['outcome']}")

    print(f"\n  UPCOMING (week={upd['week']} round={upd['round_id']}):")
    print(f"  {'#':<4} {'Home':<20} {'Away':<20} {'H':>6} {'D':>6} {'A':>6}  Pos      Pts      Form%")
    print(f"  {'-'*80}")
    for m in upd['matches']:
        print(f"  {m['n']:<4} {m['home']:<20} {m['away']:<20} {m['odds']['H']:>6} {m['odds']['D']:>6} {m['odds']['A']:>6}  {str(m['h_pos'])+'v'+str(m['a_pos']):^8} {str(m['h_pts'])+'v'+str(m['a_pts']):^8} {str(m['h_form'])+'v'+str(m['a_form'])}")


def monitor():
    last_week = None
    print("🔄 BongoBongo VFL Collector started\n")
